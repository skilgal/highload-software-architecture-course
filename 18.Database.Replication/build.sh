...
until docker-compose exec mysql_slave_biba sh -c 'export MYSQL_PWD=111; mysql -u root -e ";"'
do
    echo "Waiting for mysql_slave_biba database connection..."
    sleep 4
done

until docker-compose exec mysql_slave_boba sh -c 'export MYSQL_PWD=111; mysql -u root -e ";"'
do
    echo "Waiting for mysql_slave_boba database connection..."
    sleep 4
done
...
docker exec mysql_slave_biba sh -c "$start_slave_cmd"
docker exec mysql_slave_boba sh -c "$start_slave_cmd"

docker exec mysql_slave_biba sh -c "export MYSQL_PWD=111; mysql -u root -e 'SHOW SLAVE STATUS \G'"
docker exec mysql_slave_boba sh -c "export MYSQL_PWD=111; mysql -u root -e 'SHOW SLAVE STATUS \G'"
